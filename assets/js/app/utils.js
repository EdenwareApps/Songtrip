...
function rgb2arr(color){
    console.log('rgb2arr', color, traceback())
    return color.replace("rgb(", "").replace(")", "").split(",").map(s => Number(s.trim()));
}

function arr2rgb(color){
...
var tray, nwin = nw.Window.get()

function prepareTray(){
    if(!tray){
        const title = 'Songtrip'
        tray = new nw.Tray({
            title, 
...
nwin.on('restore', removeFromTray)
nwin.on('close', function (){
    if(window.isScanning){
        console.error('TRAYING')
        this.close(true)
        goToTray()
    } else {
        nwin.close(true)
