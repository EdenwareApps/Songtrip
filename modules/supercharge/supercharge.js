function patch(scope){
	...
	if (!scope.Array.prototype.sum) {
		scope.Array.prototype.sum = function() {
	...
	scope.prepareSlashes = (file, forceBackslashes) => {
		if(!forceBackslashes && !scope.pathm && typeof(require) != 'undefined') {
			scope.pathm = require('path')
		}
		const slash = (scope.pathm && !forceBackslashes) ? scope.pathm.sep : '/'
		if(slash == '/') {
			return file.replaceAll('\\', slash).replaceAll('//', slash)
		}
		return file.replaceAll('//', slash).replaceAll('/', slash)
	}
	scope.time = () => {
