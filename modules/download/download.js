...
class Download extends Events {
   constructor(opts){
	...
	connect(){
		...
		if(this.opts.debug){
			console.log('>> Download request', this.currentURL, opts, this.received, JSON.stringify(opts.headers), this.requestingRange, this.opts.headers['range'])
		}
		this.connectCount++
		let redirected
		const stream = new DownloadStream(opts)
		stream.startTime = ((new Date()).getTime() / 1000) * 1000
	...
	continue(){
		if(this.opts.debug){
			console.log('continue', this.lastStatusCode)
		}
		this.destroyStream()
		if(this.continueTimer){
			clearTimeout(this.continueTimer)
...
Download.file = (...args) => {
	let _reject, g, err, opts = args[0], file = opts && opts.file ? opts.file : require('os').tmpdir() +'/'+ Math.random()
	let promise = new Promise((resolve, reject) => {
		_reject = reject
		let writer
		g = new Download(opts)
